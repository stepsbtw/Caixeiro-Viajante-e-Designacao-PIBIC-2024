...
MAX_INSTANCE_SIZE = int(os.environ.get("MAX_INSTANCE_SIZE", 600))  # max decision variables (workers × tasks)
# Filter to only gap1-12 instances (maximization problems)
GAP_1_12_INSTANCES = [f"gap{i}_p{j}.dat" for i in range(1, 13) for j in range(1, 6)]
SMALL_INSTANCES = []
...
def get_model_file(instance_name):
    ...
    """
    import re
    if re.match(r'^gap[a-d]_p\d+\.dat$', instance_name):
...
def filter_instances_by_size(data_dir, max_size):
    """Filter .dat files to only include instances with <= max_size decision variables (workers × tasks)"""
    all_instances = [f for f in os.listdir(data_dir) if f.endswith(".dat")]
    filtered_instances = []
    
    print(f"Filtering GAP instances by size (max {max_size} variables)...")
...
def benchmark():
    ...
    # Use only gap1-12 instances (maximization problems)
    if SMALL_INSTANCES:
        test_instances = SMALL_INSTANCES
        print(f"Using predefined small instances: {test_instances}")
    else:
        # Use gap1-12 instances only
        test_instances = GAP_1_12_INSTANCES
        print(f"Using gap1-12 instances only: {len(test_instances)} instances")
